...
export function WithdrawPage() {
  ...
  return (
    <AppShell backTo="/app" width="wide">
        ...
        <motion.section variants={fadeUp} className="flex flex-col gap-3">
          ...
          <label className="flex h-14 cursor-text items-center gap-3 rounded-xl border border-asm-line bg-white px-4 shadow-[0_1px_4px_-1px_rgba(16,42,92,0.06)] focus-within:ring-2 focus-within:ring-asm-blue">
            ...
            <input
              ...
              className="min-w-0 flex-1 self-stretch bg-transparent font-mono text-base font-bold tabular-nums text-asm-navy border-0 outline-none focus:outline-none focus:ring-0 focus:shadow-none placeholder:font-sans placeholder:font-bold placeholder:text-asm-muted/50"
            />
      ...
      {/* ── Sticky submit bar ── */}
      <div
        className="fixed left-0 right-0 z-20 border-t border-asm-line bg-white/95 px-4 pb-3 pt-3 backdrop-blur-sm [bottom:calc(60px+env(safe-area-inset-bottom,0px))] lg:bottom-0 lg:pb-[max(12px,env(safe-area-inset-bottom))]"
        aria-live="polite"
      >
        <button
          type="button"
    ...
    <label className="flex flex-col gap-1.5">
      ...
      <span className="flex h-12 items-center gap-3 rounded-xl border border-asm-line bg-white px-4 shadow-[0_1px_4px_-1px_rgba(16,42,92,0.06)] focus-within:border-asm-blue focus-within:ring-2 focus-within:ring-asm-blue/20">
        ...
        <input
          ...
          className={cn(
            'min-w-0 flex-1 bg-transparent text-[13px] text-asm-navy border-0 outline-none focus:outline-none focus:ring-0 focus:shadow-none placeholder:text-asm-muted',
            className
          )}
          {...props}
        />
